...
int bSearch(int *arr, int a, int b, int key) {
    ...
    if (*(arr + mid) == key){ // Checks to see if key is the middle.
        return mid;
    } else if (*(arr + mid) < key){ // Checks to see if key is greater than middle value, than does binary search on the right portion.
        return bSearch(arr, mid + 1, b, key);
    } else { // Else does binary search on the left portion of the array, if key is less than middle value.
